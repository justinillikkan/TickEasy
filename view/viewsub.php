...
<h1 align="center" style="color:black;"><i>VIEW DETAILS</i></h1>
<?php 
$name=$_SESSION['uname'];
$sql="select * from staff where sf_name='$name'";
...
$res2=mysqli_query($con,"select * from `quali` where `Qua_id`='$qa'");
$row3=mysqli_fetch_array($res2);
	?>
	<tr>
<td>NAME/USERNAME</td><td><?php echo $row['sf_name']?></td>
</tr>
<tr>
<td>STAFF ID</td><td><?php echo $row['sf_id']?></td>
</tr>
<tr><td>DESIGNATION</td><td>	<select>			  <?php
				  
				  $requst=mysqli_query($con,"select * from desig");
				  
				 
				  while($row=mysqli_fetch_array($requst))
				  {
                   ?>
					<option value="<?php echo $row1['Desig_id'];?>"><?php echo $row1['Desig'];?></option>
...
<?php
				  ...
				  $requst=mysqli_query($con,"select * from quali");
				 
				  while($row=mysqli_fetch_array($requst))
				  {
                   ?>
					<option value="<?php echo $row3['Qua_id'];?>"><?php echo $row3['Qua_name'];?></option>
